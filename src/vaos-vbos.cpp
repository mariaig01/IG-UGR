...
void DescrVBOInds::crearVBO( )
{
   ...
   /*Esta función crea y deja activado un VBO de índices, dados los
   parámetros descriptores de la tabla de índices. También transfiere
   los datos a la GPU y fija los parámetros en OpenGL. de las diapos*/
...
void DescrVAO::crearVAO()
{
   ...
   // TODO 2. Para cada VBO de atributos adjunto al VAO (puntero en 'dvbo_atributo' no nulo):
   //       Crear el VBO de atributos en la GPU (usar 'crearVBO') 

   for(unsigned int i=0;i<num_atribs; i++){
   ...
   //TODO 4. Para cada VBO de atributos adjunto al VAO (puntero en 'dvbo_atributo' no nulo):
   //       Si la tabla está deshabilitada en el vector 'atrib_habilitado':
   //           Deshabilitarla en la GPU con 'glDisableVertexAttribArray'

      for(unsigned int i=0;i<num_atribs; i++){
         if((dvbo_atributo.at(i)!=nullptr)&&(!atrib_habilitado.at(i))){
            glDisableVertexAttribArray(dvbo_atributo.at(i)->buffer);
         }
      }
